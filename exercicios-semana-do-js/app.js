...
const numImpares = randomNumbers.filter(num => num % 2 === 1);
...
const menor501 = crazyNumbers.filter(num => num < 501);
console.log(menor501.length);

// Resposta do Professor:
const numbersLessThan501 = crazyNumbers.reduce(
  (accumulator, crazyNumber) => (crazyNumber < 501 ? accumulator + 1 : accumulator),
  0
);
...
const numbers = [5, 7, 3];
const numbersPot2 = numbers.map(number => number ** 2);
console.log(numbersPot2);
...
const moviesuntil1999 = tarantinoMovies.filter(movie => movie.release < 2000);
console.log(moviesuntil1999);
...
const namesTvShows = tvShows.map(tvShow => tvShow.name);
...
const namesGamesCart = cart.reduce((accumulator, { name }) => `${accumulator}- ${name}\n`, '');
console.log(namesGamesCart);
